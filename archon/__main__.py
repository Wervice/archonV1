...
try:
    if len(sys.argv) == 1:
        ...
        if sys.argv[1] == "sbackup":
            ...
            encrypted = "Y" == input(
                colored("Do you want to make an encrypted archive? (Y/n) ", "blue"))
            if encrypted:
                password = input(colored("Password> ", "blue"))
                overwrite = "y" == input(
                    colored("Do you want to overwrite the unencrypted version (y/N) ", "blue"))
            try:
                if is_unc_path(sys.argv[3]):
                    print(
                        colored("[-] Backup folder to network location", "blue"))
                else:
                ...
                # Print "Make the archive"
                print(colored("[-] Make compressed tar archive", "blue"))
                # Write the metadata to the backupfolder
                metadata_f_writer = open(sys.argv[2]+"archon_metadata", "w")
                metadata_f_writer.write(
                    dt_string+";\n"+str(now)+";\n"+os.getlogin())
                ...
                if encrypted:
                    pyAesCrypt.encryptFile(sys.argv[3]+"\\"+foldername+".tar.xz", sys.argv[3] +
                                           "\\"+foldername+".tar.xz.aes", passw=password, bufferSize=1024*64)
                    if overwrite:
                        print(
                            colored("[-] Overwriting unencrypted version. This may take long.", "blue"))
                ...
                # Push to the dashboard
                dashboard.push(
                    "success", sys.argv[2], sys.argv[3]+foldername+".tar.xz", True)
                dashboard.show()
            except FileNotFoundError:
            ...
            encrypted = "Y" == input(
                colored("Is the backup you want to use encrypted? (Y/n) ", "blue"))
            if encrypted:
                password = input(colored("Password> ", "blue"))
            try:
                ...
                if encrypted:
                    pyAesCrypt.decryptFile(infile=".\\temp_archon_restore_file.tmp",
                                           outfile=".\\temp_archon_restore_file.tmp.d", passw=password, bufferSize=1024*64)
                if os.path.exists(".\\temp_archon_restore_file.tmp"):
                    print(colored("[*] Got backups", "green"))
                else:
                    print(
                        colored("[x] Getting backups failed. Please try again.", "red"))
                    print("Exiting now")
                    exit()
                ...
                metadata_f_reader_ftp = open(
                    sys.argv[3]+"archon_metadata", "r")
                metadata = metadata_f_reader_ftp.read()
                metadata_f_reader_ftp.close()
                # Status message
                print(colored("[-] Remove temporary files", "blue"))
                # Remove metadata file
                os.remove(sys.argv[3]+"archon_metadata")
                # Remove downloaded file
                os.remove(".\\temp_archon_restore_file.tmp")
                if encrypted:
                ...
                print(colored("[*] Restored from backup (Backup Time " +
                              metadata.split(";\n")[0]+")", "green"))
                dashboard.push(
                    "success", sys.argv[2], sys.argv[3], folder=True)
                dashboard.show()
            except FileNotFoundError:
                print(colored(
                    "File Not Found\nPlease check your decryption password, if you used an encrypted backup.\nExiting now", "red"))
                dashboard.push("fail_filenotfound",
                               sys.argv[2], sys.argv[3], folder=True)
                dashboard.show()
            ...
            try:
                ...
                if encrypted:
                    pyAesCrypt.encryptFile(".\\archon_temp_backup_ftp_archive.tar.xz",
                                           ".\\archon_temp_backup_ftp_archive.tar.xz.aes", passw=password, bufferSize=1024*64)
                    if overwrite:
                ...
                dashboard.push(
                    "fail_filenotfound", sys.argv[2], "ftp://"+host+"/"+sys.argv[3], folder=True)
                dashboard.show()
                print(colored("File Not Found\nExiting now", "red"))
            ...
            if sys.argv[4] == "tls":
                ...
                print(
                    colored("[?] Your connection is not secure. Do you want to continue?", "red"))
                if input("Y/n> ") == "Y":
            ...
            try:
                ...
                dashboard.push(
                    "fail_permission", sys.argv[2], "ftp://"+host+"/"+sys.argv[3], folder=True)
                dashboard.show()
                exit()
            try:
                if encrypted:
                ...
                connection.storbinary("STOR %s" % (
                    backup_location),  backup_submitter)
                backup_submitter.close()
                print(colored("[*] Wrote file to server", "green"))
            except ftplib.error_perm:
                print(
                    colored("[x] You're not allowed to write the backup there", "red"))
            connection.close()
            if encrypted:
            ...
            dashboard.push(
                "success", sys.argv[2], "ftp://"+host+"/"+sys.argv[3], folder=True)
            dashboard.show()
            ...
            print(colored("[*] Connected", "green"))
            connection.retrbinary("RETR %s" % (sys.argv[2]), open(
                "temp_archon_restore_file_ftp.tmp", "wb").write)
            if encrypted:
                pyAesCrypt.decryptFile("temp_archon_restore_file_ftp.tmp",
                                       "temp_archon_restore_file_ftp.tmp.d", passw=password_archive, bufferSize=1024*63)
            try:
                print(colored("[-] Getting backups", "blue"))  # Status message
                # Download the backup
                ...
                os.remove(sys.argv[3]+"archon_metadata")
                # Remove downloaded file
                os.remove(".\\temp_archon_restore_file_ftp.tmp")
                if encrypted:
                ...
                print(
                    colored("Permission Error. Please check your password and username.", "red"))
except IndexError:
    print(colored("You need more arguments.\nExiting now", "red"))
except ftplib.error_perm:
    print(colored("FTP Error", "red"))
    dashboard.push("fail_permission", sys.argv[2], "ftp://"+host+"/"+sys.argv[3], folder=True)
    dashboard.show()
